...
$conn=new mysqli("localhost","root","","addressbook") OR die("Error: ".mysqli_error($conn));
 session_start();
...
if(isset($_POST['save'])) {
    if(!empty($_POST['name']) && !empty($_POST['date']) && !empty($_POST['phone']) && !empty($_POST['email']))
    {
        ...
        $email=$_POST['email'];
            ...
            $SELECT="SELECT phone From account Where phone = ? Limit 1";
            $INSERT="INSERT Into account (name,date,phone,email) values(?,?,?,?)";
...
if(isset($_POST['edit'])) {
    if(!empty($_POST['name']) && !empty($_POST['date']) && !empty($_POST['phone']) && !empty($_POST['email']))
    {
        ...
        $uQuery="UPDATE account SET name=?, date=?,phone=?,email=? WHERE id=?";

        
          

            $stmt=$conn->prepare($uQuery);
            $stmt->bind_param("ssssi",$name,$date,$phone,$email,$id);
            if($stmt->execute())
            {
...
if(isset($_POST['submit2']))
{
    ...
    $ssm=mysqli_query($conn,"SELECT * FROM account where Name='name' ");
